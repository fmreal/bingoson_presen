...
s.dynamicAccountList="calpisdev=www.virtualcom.co.jp;calpiscojp=www.calpis-shop.jp,www.calpis.co.jp"
/* Conversion Config */
s.currencyCode="JPY"
...
s.linkInternalFilters="javascript:,calpis.co.jp,calpis-sour.jp,pr-calpis.jp,calpissoda.jp,sodacup.jp,fruit-calpis.jp,calcam.jp,calcolor.jp,c-sour.com,aminovital-yell.jp,hotlemon.jp,calpis-m.com,calpis-cfs.co.jp,calpis-cb.co.jp,ameals.net,calpis.net,calpico.com,calsporin.com,calpis-shop.jp,gggcam.jp,calpis.jp,calpiswater.jp,calpis-water.jp,laclac.info,cw-diary.com,premium-calpis.jp,c-sour.jp,soda-kenpo.jp"
...
function s_doPlugins(s) {
	var sc_Section,sc_Domain;
	/* Add calls to plugins here */
	/* Set PageURL */
	s.eVar19=s.prop18=((location.protocol=="https")?"http:":location.protocol) + "//" + location.host + location.pathname;
	...
	if (s.getNewRepeat()=="New") {
		s.prop12 = s.eVar12 = "NewVisitor";
		s.prop13 = s.eVar13 = location.pathname.split("/").slice(1,-1).join(":");
	}else {
		s.prop12 = s.eVar12 = "Repeater";
		s.prop14 = s.eVar14 = location.pathname.split("/").slice(1,-1).join(":");
	}

	var ppvArray = s.getPercentPageViewed(s.pageName);
	s.prop22 = ppvArray[0] //contains the previous page name
	s.prop23 = ppvArray[1] //contains the total percent viewed
	s.prop24 = ppvArray[2] //contains the percent viewed on initial load
	s.prop25 = ppvArray[3] //contains the total number of vertical pixels

	/* To send the hit on an exit link multiple values*/
	var url = s.exitLinkHandler()
	if(url) {
		s.linkTrackVars = 'prop22,prop23,prop24,prop25';
		var ppvArray = s.getPercentPageViewed(s.pageName);
		s.prop22 = ppvArray[0]
		s.prop23 = ppvArray[1]
		s.prop24 = ppvArray[2]
	...
	s.prop17 = s.eVar17 = s.getAndPersistValue(s.campaign,'s_getval_cid',7)+":" + location.pathname;
	s.events=s.apl(s.events,'event18',',',1);
	
	// Set hierarchical data
	sc_Section=location.pathname.split("/").slice(1,-1).join(":");
	sc_Domain=location.hostname;
	s.hier1=sc_Domain + ":" + sc_Section;
	
	// Get Smart Device String
	if (navigator.userAgent.indexOf('iPhone') > 0) {
		s.eVar20=s.prop20="iPhone";
	} else if ( navigator.userAgent.indexOf('iPad') > 0 ) {
		s.eVar20=s.prop20="iPad";
	} else if ( navigator.userAgent.indexOf('Android') > 0 ) {
		s.eVar20=s.prop20="Android";
	}
...
/*
 * Plugin: getQueryParamJP - VCC original
 */
function getQueryParamJP(param) {
    ...
    str2 = str.substring(start+param.length+1,end);
    return decodeJP(str2);
}
function decodeJP(_data) {
    var dStr = "";
...
s.visitorNamespace="calpis"
s.trackingServer="calpis.112.2o7.net"
